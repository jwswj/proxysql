...
const char* JSON_STR="json";

log_event_format parse_fileformat(char* str) {
    if (str == NULL || strcmp(BINARY_STR, str) == 0) {
        return BINARY;
    } else if (strcmp(JSON_STR, str) == 0) {
		return JSON;
	} else if (strcmp(FORMAT_UNSPECIFIED_STR, str) == 0) {
    	return FORMAT_UNSPECIFIED;
	} else {
		proxy_warning("Invalid query log format '%s'\n", str);
